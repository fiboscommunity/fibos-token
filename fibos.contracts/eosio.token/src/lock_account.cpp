...
void token::exlock(account_name owner, extended_asset quantity, time_point_sec expiration, string memo)
{
    ...
    eosio_assert(st.connector_weight == 0, "only classic token can be locked");
...
void token::exunlock(account_name owner, extended_asset quantity, time_point_sec expiration, string memo)
{
    ...
    if (st.connector_weight == 0) {
        ...
        account_name foundation = get_foundation(quantity.contract);

        if (owner != foundation) {
            extended_asset balance = get_balance(quantity, foundation);
            if (balance >= quantity) {
                sub_balance(foundation, quantity);
                lock_add_balance(foundation, quantity, time_point_sec(), owner);
            } else {
                extended_asset tokens = unlock_with_locked_tokens(quantity - balance);
                sub_balance(foundation, balance);
                lock_add_balance(foundation, balance, time_point_sec(), owner);
                lock_sub_balance(foundation, tokens);
            }
        } else {
            extended_asset foundation_pay = unlock_with_connector(quantity, false);
            extended_asset connector_balance = get_balance(foundation_pay, foundation);

            if (connector_balance >= foundation_pay) {
                foundation_pay = unlock_with_connector(quantity, true);
                sub_balance(foundation, foundation_pay);
            } else {
                extended_asset tokens = unlock_with_connector(connector_balance, quantity);
                tokens = unlock_with_locked_tokens(quantity - tokens);
                sub_balance(foundation, connector_balance);
                lock_sub_balance(foundation, tokens);
            }
        }
        add_balance(owner, quantity, owner);
